...
inline cudnnStatus_t cudnnConvolutionForward(cudnnHandle_t handle,
  ...
  assert(miopenStatusSuccess == miopenFindConvolutionForwardAlgorithm(handle, xDesc, x, wDesc, w, convDesc, yDesc, y, 1, &returnedAlgoCount, &perfResults, workSpace, workSpaceSizeInBytes, false));
  return miopenConvolutionForward(handle, alpha, xDesc, x, wDesc, w, convDesc, perfResults.fwd_algo, beta, yDesc, y, workSpace, workSpaceSizeInBytes);
}
...
inline cudnnStatus_t cudnnGetConvolutionForwardWorkspaceSize(
  ...
  int stride[4], wshape[4], xshape[4], yshape[4];
  ...
  ensure_cmd(miopenStatusSuccess == cudnnGetTensor4dDescriptor(yDesc, &dtype, yshape + 0, yshape + 1, yshape + 2, yshape + 3, stride + 0, stride + 1, stride + 2, stride + 3) && dtype == miopenFloat);

  *sizeInBytes = wshape[1] * wshape[2] * wshape[3] * yshape[2] * yshape[3] * sizeof(float) * WS_BOUND;
  // size_t w2 = 0; miopenConvolutionForwardGetWorkSpaceSize(handle, wDesc, xDesc, convDesc, yDesc, &w2); printf("%zd v.s. %zd\n", *sizeInBytes, w2);
